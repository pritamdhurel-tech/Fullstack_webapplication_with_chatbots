  ...
  return (
    <div className="flex gap-1 mb-4">
      {Array.from({ length: 5 }).map((_, i) => (
        <span
          key={i}
          className={`text-sm ${i < rating ? "text-yellow-400" : "text-[#4A4F66]"}`}
        >
          ★
        </span>
      ))}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitState({
        loading: false,
        error: "",
        success: "Thank you! Your feedback is pending verification.",
      });
      setTimeout(() => {
        setShowModal(false);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-14 gap-6">
        <div>
          <p className="text-[#8A8FA8] text-base leading-relaxed max-w-xl">
            Feedback submitted following project completion. Ratings are
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading
          ? Array.from({ length: 4 }).map((_, i) => (
              <SkeletonCard key={i} type="feedback" />
            ))
          : feedback.map((f, i) => (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-200 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent/50"
                ...
                <div>
                  ...
                  <select
                    ...
                    value={form.rating}
                    onChange={handleChange}
                    className="w-full border border-gray-200 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent/50 bg-white"
                  >
                    {[5, 4, 3, 2, 1].map((r) => (
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows="4"
                    className="w-full border border-gray-200 rounded-lg px-4 py-2 text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent/50 resize-none"
