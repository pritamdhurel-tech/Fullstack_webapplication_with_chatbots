...
export default function AdminArticles() {
  ...
  async function handleSave() {
    ...
    try {
      const url = editId
        ? buildApiUrl(`/api/articles/${editId}`)
      ...
      closeModal();
      setSuccess(editId ? "Article updated." : "Article created.");
      load();
  ...
  async function handleDelete(id) {
    ...
    await fetch(buildApiUrl(`/api/articles/${id}`), {
      method: "DELETE",
      headers,
    });
  ...
  return (
    <div>
      <PageHeader
        title="Articles"
        subtitle="Manage promotional articles on the public site"
        action={
          <button
      ...
      <Table headers={["Title", "Category", "Author", "Actions"]}>
        {items.map((item) => (
          <Tr key={item.id}>
            <Td className="font-medium text-[#F0F0F5] max-w-xs truncate">
              {item.title}
            </Td>
            <Td>{item.category ?? "—"}</Td>
            <Td>{item.author ?? "—"}</Td>
        ...
        >
          ...
          <FormField label="Excerpt *">
            <Textarea
              ...
              value={form.excerpt}
              onChange={handleChange}
              placeholder="Short summary shown on the articles page..."
            />
          </FormField>
          <FormField label="Full content">
            <Textarea
              ...
              value={form.content}
              onChange={handleChange}
              placeholder="Full article body (optional)..."
            />
          </FormField>
