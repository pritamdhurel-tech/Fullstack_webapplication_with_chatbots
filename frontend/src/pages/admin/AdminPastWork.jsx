...
export default function AdminPastWork() {
  ...
  async function load() {
    const res = await fetch(buildApiUrl("/api/past-work"));
  ...
  async function handleSave() {
    if (!form.title || !form.industry || !form.description) {
      setError("Title, industry, and description are required.");
      return;
    }
    try {
      const url = editId
        ? buildApiUrl(`/api/past-work/${editId}`)
        : buildApiUrl("/api/past-work");
  ...
  async function handleDelete(id) {
    ...
    await fetch(buildApiUrl(`/api/past-work/${id}`), {
      method: "DELETE",
      headers,
    });
  ...
  return (
    <div>
      <PageHeader
        title="Past Work"
        subtitle="Manage case studies shown on the public site"
        action={
          <button
      ...
      {modal && (
        <Modal
          title={modal === "create" ? "Add past work" : "Edit past work"}
          onClose={closeModal}
        >
