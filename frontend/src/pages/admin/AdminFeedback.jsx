...
export default function AdminFeedback() {
  ...
  async function load() {
    try {
      const res = await fetch(buildApiUrl("/api/feedback/admin"), { headers });
      if (!res.ok) {
  ...
  async function handleSave() {
    if (!form.feedback_text || !form.customer_name) {
      setError("Feedback text and customer name are required.");
      return;
    }
    try {
      const url = editId
        ? buildApiUrl(`/api/feedback/${editId}`)
      ...
      closeModal();
      setSuccess(editId ? "Feedback updated." : "Feedback created.");
      load();
  ...
  async function handleDelete(id) {
    ...
    await fetch(buildApiUrl(`/api/feedback/${id}`), {
      method: "DELETE",
      headers,
    });
  ...
  async function handleToggleApprove(id, currentStatus) {
    if (
      !confirm(
        `Are you sure you want to ${currentStatus ? "reject" : "approve"} this feedback?`,
      )
    )
      return;
    try {
      const res = await fetch(buildApiUrl(`/api/feedback/${id}/approve`), {
        method: "PATCH",
        headers,
      });
      if (!res.ok) throw new Error();
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Manage testimonials displayed on the public site"
        action={
          <button
      ...
      <Table headers={["Customer", "Rating", "Status", "Feedback", "Actions"]}>
        {items.map((item) => (
          <Tr key={item.id}>
            ...
            <Td>
              <span
                className={`px-2 py-1 rounded text-xs font-semibold ${item.is_approved ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}
              >
      ...
      {modal && (
        <Modal
          title={modal === "create" ? "Add feedback" : "Edit feedback"}
          onClose={closeModal}
        >
