...
export default function AdminSolutions() {
  ...
  async function handleSave() {
    ...
    try {
      const url    = editId ? `/api/solutions/${editId}` : '/api/solutions'
  ...
  async function handleDelete(id) {
    ...
    await fetch(`/api/solutions/${id}`, { method: 'DELETE', headers })
  ...
  return (
    <div>
      ...
      {modal && (
        <Modal title={modal === 'create' ? 'Add solution' : 'Edit solution'}
               onClose={closeModal}>
          ...
          <div className="flex gap-3 justify-end mt-2">
            <button className="btn-ghost text-sm px-5 py-2" onClick={closeModal}>
              Cancel
            </button>
            <button className="btn-primary text-sm px-5 py-2" onClick={handleSave}>
